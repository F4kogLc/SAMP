...
function SE.onAimSync(pId, data)
    for k, v in pairs(camModes) do
        if data.camMode == v then
            sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Detonator crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
            return false
        end
    end
end

function SE.onVehicleSync(pId, vId, data)
    local pos, inf = data.position, 1/0
    if pos.x == inf or pos.y == inf or pos.z == inf then
        sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Vortex crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
        return false
    end
end

function SE.onTrailerSync(pId, data)
    if isCharInAnyCar(playerPed) then
        _, v = sampGetVehicleIdByCarHandle(storeCarCharIsInNoSave(playerPed))
        if data.trailerId == v then
            sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Trailer crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
            return false
        end
    end
end

function SE.onUnoccupiedSync(pId, data)
    if data.roll.x>=10000.0 or data.roll.y>=10000.0 or data.roll.z>=10000.0 or data.roll.x<=-10000.0 or data.roll.y<=-10000.0 or data.roll.z<=-10000.0 then
        sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Roll crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
        return false
    end
end

function SE.onUnoccupiedSync(pId, data)
    if data.position.x>=19900.0 or data.position.y>=19900.0 or data.position.z>=19900.0 or data.position.x<=-19900.0 or data.position.y<=-19900.0 or data.position.z<=-19900.0 then
        sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Position crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
        return false
    end
end

function SE.onPlayerSync(pId, data)
    if data.weapon == 40 and data.keysData == 128 then
        sampAddChatMessage('{1560bd}[AntiCrasher]{FFFFFF} > %s[%d] > Invisible crasher was used on you by '..sampGetPlayerNickname(pId)..'['.. pId ..']')
        emul_rpc('onPlayerStreamOut', { pId })
        return false
    end
end
